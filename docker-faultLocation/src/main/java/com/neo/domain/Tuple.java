...
public class Tuple {
  ...
  public String toString() {
	...
	for (int i = 0; i < testCase.getLength(); i++) {
	  if (data[i] == -1)
		...
		rs.append(" ").append(data[i]).append(" ");
	  if (i != testCase.getLength() - 1)
		rs.append(",");
	}
	return rs.toString().substring(1, rs.length() - 1);
  }
  ...
  private List<Tuple> getChildTuplesByDegree(int degree) {
	// TODO Auto-generated method stub
	List<Tuple> result = new ArrayList<Tuple>();
...
//			} 
  ...
  private List<Tuple> getFatherTuplesByDegree(int degree) {
	List<Tuple> result = new ArrayList<Tuple>();
  ...
  private Tuple cat(Tuple A, Tuple B) {
